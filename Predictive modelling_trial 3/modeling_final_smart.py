...
def run_smart_modeling():
    print("==============================================")
    print("   MODUL K-MEANS: TRIAL & ERROR (FORCE k=3)   ")
    print("==============================================")
    ...
    # Variabel untuk menyimpan skor tertinggi secara matematis (hanya untuk info)
    math_best_score = -1
    math_best_k = 0
    ...
    for k in k_range:
        ...
        # Cek skor matematis (hanya untuk catatan)
        if score > math_best_score:
            math_best_score = score
            math_best_k = k
            
        print(f"   [TRIAL] Testing k={k} cluster... | Inertia: {inertia:.0f} | Silhouette Score: {score:.4f}")

    print("-" * 60)
    print(f"   > INFO MATEMATIS: Secara murni statistik, k terbaik adalah {math_best_k} (Score: {math_best_score:.4f})")
    
    # --- MODIFIKASI: MEMAKSA PILIH 3 ---
    # Kita mengabaikan saran matematika dan memilih k=3 demi kepentingan Bisnis/Strategi
    best_k = 3
    print(f"   > KEPUTUSAN BISNIS: Kita memilih k={best_k} untuk segmentasi yang lebih actionable.")
    ...
    # Menandai titik pilihan kita (k=3) di grafik
    plt.axvline(x=best_k, color='green', linestyle=':', label=f'Selected (k={best_k})')

    plt.title(f'Evaluasi Cluster (Dipilih: k={best_k})')
    fig.tight_layout()
    ...
    # Jalankan ulang K-Means dengan k=3
